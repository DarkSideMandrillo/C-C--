...
void test2()
{
    ...
    vec.shrink_to_fit(); // C++11 //Serve per fittare la capacità del vec alla reale grandezza
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;

    vec.reserve(100); // Riserva una quantità specifica di spazio
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;
...
void test6()
{
    ...
    display(vec);
    vec.erase(vec.begin(), vec.begin() + 2); // Distrugge un pezzo del vettore (i primi due)
    display(vec);
    ...
    {
        if (*it % 2 == 0)
            vec.erase(it); // Cancella la posizione del vettore dell'iteratorore
        else
...
void test9()
{
    ...
    std::copy(vec1.begin(), vec1.end(), std::back_inserter(vec2));  //Copia vec1 (scorrendolo) all'inizio(algoritm) di vec2
    display(vec1);
    display(vec2);
    std::cout << std::endl;
...
void test11()
{
    ...
    auto it = std::find(vec1.begin(), vec1.end(), 5);  //trova il 5 nel vec1
    if (it != vec1.end())  
    {
